...
class SACAgent:
	def __init__(
		self, observation_space, action_space, hidden_size=256, hidden_layers=3
	):
		...
		self.critic_params = list(
			self.network.critic1_output.parameters()
	...
	def update_critics(
		...
		with torch.no_grad():
			next_actions, next_log_probs = self.network.sample_action(
				next_states
			)
			target_q1, target_q2 = self.target_network.forward_critic(
